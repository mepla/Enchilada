...
class UserCheckins(Resource):
    ...
    # /users/<string:target_uid>/checkins
    def get(self, target_uid, uid):
        ...
        try:
            ...
            if include_business_info:
                business_infos = {}
                for checkin in checkins:
                    checkin_bid = checkin.get('bid')
                    try:
                        existing_biz = business_infos.get(checkin_bid)
                        if not existing_biz:
                            existing_biz = self.doc_db.find_doc('bid', checkin_bid, doc_type='business', limit=1)
                            business_infos[checkin_bid] = self.filter_business_info_for_checkins(existing_biz)
                        checkin['business'] = business_infos.get(checkin_bid)
                    except Exception as exc:
                        logging.error('Could not fetch user info in User checkins: {}'.format(exc))

            return filter_general_document_db_record(checkins)
            ...
            msg = {'message': 'There are no checkins.'}
            logging.info(msg)
            return msg, 204

    def filter_business_info_for_checkins(self, business_info):
        filtered_info = {}
